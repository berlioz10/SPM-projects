...
void send_to_the_other_comm(long a, long b, long c, long d, int master_comm) {
	std::vector<long> list = {a, b, c, d};
	MPI_Send(list.data(), 4, MPI_LONG, master_comm, 0, MPI_COMM_WORLD);
}
...
float compute(const long c1, const long c2, long key1, long key2, int myId, int numP, MPI_Comm mm_comm) {
    ...
    if(myId == 0) {
        ...
        for(int i = 1; i < numP; i ++) {
			...
			MPI_Send(send_to.data(), 3, MPI_LONG, i, 0, mm_comm);

            MPI_Send(&A[start_Id * c2], how_many * c2, MPI_FLOAT, i, 0, mm_comm);
            MPI_Send(B.data(), c2 * c1, MPI_FLOAT, i, 0, mm_comm);
        }
		r = mm(A, B, c1, c2, end);
    }
    else {
		...
		MPI_Recv(recv.data(), 3, MPI_LONG, 0, 0, mm_comm, MPI_STATUS_IGNORE);
		if(recv[0] == -1) {
			return -1.0;
		}
		b1 = recv[0];
		b2 = recv[1];
		how_many = recv[2];
		// std::printf("%d: %ld %ld %ld\n", myId, b1, b2, how_many);
		A = std::vector<float>(how_many * b2, 0.0);
		B = std::vector<float>(b2 * b1, 0.0);
        MPI_Recv(A.data(), how_many * b2, MPI_FLOAT, 0, 0, mm_comm, MPI_STATUS_IGNORE);
		MPI_Recv(B.data(), b2 * b1, MPI_FLOAT, 0, 0, mm_comm, MPI_STATUS_IGNORE);
		
		r = mm(A, B, b1, b2, how_many);
    }
    float sum = 0.0;
    MPI_Reduce(&r, &sum, 1, MPI_FLOAT, MPI_SUM, 0, mm_comm);
...
int main(int argc, char* argv[]) {
	...
	color = myId % 2;
	MPI_Comm mm_comm;
	MPI_Comm_split(MPI_COMM_WORLD, color, myId, &mm_comm);

	int newId, newNumP;
	MPI_Comm_rank(mm_comm, &newId);
	MPI_Comm_size(mm_comm, &newNumP);
	...
	if(myId == 0) {
		...
		if(true) {
			for(int i=0; i<length; ++i) {
				...
				if (map[key1] == SIZE && map[key2]!=0) {
					...
					send_to_the_other_comm(map[key1], map[key2], key1, key2, 1);
					resetkey1=true;	
				}
				// if key2 reaches the SIZE limit ....
				if (map[key2] == SIZE && map[key1]!=0) {			
					r2= compute(map[key2], map[key1], key2, key1, newId, newNumP, mm_comm);
					V[key2] += r2;  // sum the partial values for key1
					resetkey2=true;
				}
				if (resetkey1) {
					MPI_Recv(&r1, 1, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
					V[key1] += r1;
			...
			for(long i=0;i<nkeys; ++i) {
				for(long j=0;j<nkeys; ++j) {
					if (i==j) continue;
					if (map[i]>0 && map[j]>0) {
						send_to_the_other_comm(map[i], map[j], i, j, 1);
						auto r2= compute(map[j], map[i], j, i, newId, newNumP, mm_comm);
						float r1;
						MPI_Recv(&r1, 1, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
						V[i] += r1;
						V[j] += r2;
		...
		send_to_the_other_comm(-1, -1, -1, -1, 1);

		for(int i = 1; i < newNumP; i ++) {
			MPI_Send(exit_vector.data(), 3, MPI_LONG, i, 0, mm_comm);
		}
	} else {
		bool ok = true;
		while(ok) {
			if(newId == 0) {
				std::vector<long> l(4);
				MPI_Recv(l.data(), 4, MPI_LONG, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				if(l[0] == -1) {
					ok = false;
					std::vector<long> exit_vector = {-1, -1, -1};
					for(int i = 1; i < newNumP; i ++) {
					...
					float r = compute(l[0], l[1], l[2], l[3], newId, newNumP, mm_comm);
					MPI_Send(&r, 1, MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
				}
			}
			else {
				float r = compute(0, 0, 0, 0, newId, newNumP, mm_comm);
				if(r == -1.0)
					ok = false;
			}
		}
