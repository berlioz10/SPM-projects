...
void compute_file(const std::string& filename, std::vector<umap>& vUM) {
	{
		...
		if (file.is_open()) {
			std::string line;
				while(std::getline(file, line)) {
					...
					#pragma omp task shared(vUM) firstprivate(line)
					if (!line.empty()) {
						tokenize_line(line, vUM[omp_get_thread_num()]);
					}
				}
			}
		file.close();
...
int main(int argc, char *argv[]) {
	...
	#pragma omp parallel firstprivate(num_threads) shared(vUM, filenames) default(none)
	{
		#pragma omp single
		...
		for(int i = 2; i <= num_threads; i*=2)
		{
			for(int j = 0; j < num_threads / i; j++) {
				#pragma omp task
				{
					reduce_maps(vUM[j], vUM[num_threads / (i / 2) - (j + 1)]);
				}
			}
			#pragma omp taskwait
		}
